...
PUBLISH_PRICES_INTEGRATION_TEMPLATE = {
    ...
    'files_search_request_data': {'sEcho': '1',
'iColumns': '5',
'sColumns':',,,,',
'iDisplayStart': '0',
...
def new_session(auth=False, chain_data=None):
    ...
    if auth:
        session = authenticate_cerberus_web_client(session, chain_data['url'], chain_data['login_post_url'], chain_data['username'], chain_data["password"])
    return session
...
def authenticate_cerberus_web_client(session, url, post_login_url, username, password="", verify=False):
    ...
    try:		
        csrf_value = soup.find('meta', {'name': 'csrftoken'}).get('content')
    except:
        print("[!] Error while trying to grab csrftoker hidden input")
    auth_payload["csrftoken"] = csrf_value

    post = session.post(post_login_url, verify=False, data=auth_payload)
    soup = BeautifulSoup(post.content, "html.parser")
    try:
        csrf_token= soup.find('meta', {'name': 'csrftoken'}).get('content')
    except:
        print("[!] Error while trying to grab csrftoker hidden input")
    return session
...
def get_full_price_file_name(session, chain_data, store_id=None):
    ...
    global csrf_token
    chain_data['files_search_request_data']['csrftoken'] = csrf_token
    req_data = session.post('https://url.retail.publishedprices.co.il/file/json/dir', verify=False, data=chain_data['files_search_request_data'])
